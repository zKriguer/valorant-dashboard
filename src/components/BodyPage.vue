<template>
  <main class="main">
    ...
    <div class="bodystats" id="statsbody">
      <div class="principal">
        {{content()}}
      </div>
    </div>
  ...
  methods: {
    content() {
      const clear = (something) => {
        while (something.firstChild !== null) {
        ...
        h1.textContent = `${this.$store.state.usuario.name}'s last matches`;
      };
      ...
      this.$store.state.partidas.forEach((partida, index) => {
        //SCOPED VARIABLES
        let modo = partida.metadata.mode;
        let jogador = partida.players.all_players.find((jogador) => {
          return jogador.name === this.$store.state.usuario.name;
        });
        let jogadorVenceu =
          partida.teams[
        ...
        let tipoPartida = document.createElement("h2");
        let personagemNome = document.createElement("p");
        let result = document.createElement("p");
        let line = document.createElement("div");
        let agentImageDiv = document.createElement("div");
        let personagemAvatar = document.createElement("img");
        ...
        duracao.textContent =
          (partida.metadata.game_length / 60000).toFixed(0) + " Minutes";
        ...
        fixed.appendChild(arrow);
        fixed.appendChild(duracao);
        fixed.appendChild(mapa);
        fixed.appendChild(tipoPartida);
        fixed.appendChild(personagemNome);
        fixed.appendChild(agentImageDiv);
        fixed.appendChild(line);
        fixed.appendChild(result);
        agentImageDiv.appendChild(personagemAvatar);

        this.desc(partida, matchData, modo);
      });
    },
    desc(partida, matchData, modo) {
      ...
      matchData.innerHTML = `
        <div class="data">
          <div class="info mobile"> ${this.mapa} <p>Map: ${
        partida.metadata.map
      }</p> </div>
          <div class="info"> ${this.elo} <p>Elo: ${
        jogador.currenttier_patched
      }</p> </div>
          <div class="info"> ${this.kills} <p>Kills: ${
        jogador.stats.kills
      } </p> </div>
          <div class="info">${this.assists}<p>Assists: ${
        jogador.stats.assists
      } </p> </div>
          ${
            modo.toLowerCase() == "deathmatch"
              ? ``
              : ` <div class="info"> ${this.spent}<p>Spent: ${jogador.economy.spent.overall} </p> </div>`
          } 
          ${
            modo.toLowerCase() == "deathmatch"
              ? ``
              : ` <div class="info"> ${this.spent}<p>Average Spent: ${jogador.economy.spent.average} </p> </div>`
          } 
          <div class="info"> ${this.rounds} <p> Rounds: ${
        partida.metadata.rounds_played
      } </p> </div>
          ${
            modo.toLowerCase() != "deathmatch"
              ? ``
              : `<div class="info">${this.score}<p> Score: ${jogador.stats.score} </p> </div>`
          }
          
        </div>
        <div class="data">
          <div class='info mobile'> ${this.timer} <p>Duracao: ${
        (partida.metadata.game_length / 60000).toFixed(0) + " Minutes"
      } </p>  </div>
          <div class="info">${this.level} <p> Level: ${
        jogador.level
      } </p> </div>
          <div class="info"> ${this.heads} <p> Head Shots: ${
        jogador.stats.headshots
      } </p> </div>
          <div class="info"> ${this.body} <p> Body Shots: ${
        jogador.stats.bodyshots
      } </p> </div>
          <div class="info"> ${this.legs} <p> Leg Shots: ${
        jogador.stats.legshots
      } </p> </div>
          <div class="info">${this.kills} <p>Deaths: ${
        jogador.stats.deaths
      } </p> </div>
          ${
            modo.toLowerCase() == "deathmatch"
              ? ``
              : `<div class="info"> ${this.damage} <p>Damage Made: ${jogador.damage_made} </p></div>`
          }
        </div>
        `;
      if (modo.toLowerCase() == "deathmatch") {
      ...
      jogadores.forEach((jogador) => {
        if (jogador.team.toLowerCase() == "blue") {
          ...
          nickTag.innerHTML = `<div> <img src=${jogador.assets.agent.killfeed} alt"" > <p class='jogadores'> ${jogador.name}#${jogador.tag} </p> </div>  <p class = 'kd'>K/D=${jogador.stats.kills}/${jogador.stats.deaths}</p>`;
          playersBlue.appendChild(nickTag.cloneNode(true));
        } else if (jogador.team.toLowerCase() == "red") {
          nickTag.className = "containerRed";
          nickTag.innerHTML = `<p class = 'kd'>K/D=${jogador.stats.kills}/${jogador.stats.deaths} </p> <div> <p class='jogadores'> ${jogador.name}#${jogador.tag} </p> <img src=${jogador.assets.agent.killfeed} alt"" > </div>`;
...
.fixed {
  ...
  overflow: hidden;
  flex-flow: column;
  box-shadow: 0 0 10px 0 #0e1822, 10px 10px 0 0 #0e1822;
}
.vermais.selected .girinho {
...
.vermais {
  ...
  max-width: 640px;
  margin: 20px 0px;
  transition: box-shadow 0.6s ease-out, max-height 0.6s ease-out;
  margin: 30px 30px;
}
.vermais.selected {
  box-shadow: 0 0 10px 0 #0e1822, 10px 10px 0 0 #0e1822;
...
.vermais.selected .matchdata {
  max-height: 670px;
}
.vermais.selected .fixed {
  box-shadow: 0 0 10px 0 #0e1822, 0px 10px 0 0 #0e1822;
}
.fixed h2 {
  position: absolute;
  top: -10px;
